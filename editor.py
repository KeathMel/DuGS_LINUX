...
from canvas import Canvas, node_pixmap, paint_canvas_background
from editor_settings import SettingsPanelMixin
from home_screen import load_home_ui_settings, register_themed_screen, DEFAULT_LOGO_COLOR
...
class Editor(QWidget, SettingsPanelMixin):
    def __init__(self, app):
        ...
        home = QLabel("DuGS")
        home.setStyleSheet(
            f"color:{self.settings.get('logo_color', DEFAULT_LOGO_COLOR)}; "
        ...
        self.deploy_btn.setToolTip(
            "Save this workflow to the API's data volume and hot-register its\n"
            "webhook/schedule triggers — from then on the API runs it on its own,\n"
            "no need to keep this editor open."
        )
        self.deploy_btn.clicked.connect(self.deploy)
        save_btn = QPushButton("Save"); save_btn.clicked.connect(self.save)
        for b in (save_btn, self.sim_btn, self.deploy_btn, run_btn): bar.addWidget(b)
        cc.addLayout(bar)
    ...
    def apply_theme(self):
        """Called by the settings broadcast when the shared logo color (or
        anything else on either settings page) is changed — and also called
        by _apply_project_kind() for normal projects, so the logo always
        resyncs to the shared "DuGS" text/color rather than drifting."""
        logo_color = self.settings.get("logo_color", DEFAULT_LOGO_COLOR)
        self.logo.setText("DuGS")
        self.logo.setStyleSheet(
            f"color:{logo_color}; font-family:monospace; font-size:20px; font-weight:bold;"
        )
        self.update()  # repaint background behind the side panels too

    def paintEvent(self, event):
        """Paints the same flat-grey/image background behind the whole
        editor (settings/projects/log on the left, palette/JSON on the
        right) as the canvas uses in the middle — so when a canvas
        background is on, the side panels match it instead of staying
        plain/transparent. Skipped when canvas_no_background is on."""
        if not self.settings.get("canvas_no_background", False):
            painter = QPainter(self)
            painter.setRenderHint(QPainter.RenderHint.Antialiasing)
            paint_canvas_background(self, painter, self.settings)
            painter.end()
    ...
    def _apply_project_kind(self):
        ...
        self.deploy_btn.setVisible(not servo)
        if servo:
            ...
            # restore the shared "DuGS" look (text + logo_color) instead of
            # the old hardcoded lowercase/blue — this is what was making the
            # logo drift out of sync with the home screen every time a
            # normal project was opened
            if hasattr(self, "logo"):
                self.apply_theme()
    ...
    def deploy(self):
        """Sends the current workflow to the API's /projects/deploy endpoint:
        the API saves it into its own data volume and hot-registers any
        webhook/schedule triggers. From that point on the API runs this
        workflow entirely on its own — this editor doesn't need to stay open."""
        if not self.current_project:
            return
        if getattr(self, "project_kind", "normal") == "servo":
            self.results.setText("servo projects don't deploy — use Export Code instead")
            return
        self.save()  # make sure the local copy is current before sending it
        wf = self.canvas.to_workflow(self.current_project)
        wf["kind"] = getattr(self, "project_kind", "normal")
        try:
            resp = api_post("/projects/deploy", wf)
            if resp.get("ok"):
                hooks = resp.get("registered_webhooks") or []
                scheds = resp.get("registered_schedules") or []
                bits = []
                if hooks: bits.append(f"{len(hooks)} webhook(s)")
                if scheds: bits.append(f"{len(scheds)} schedule(s)")
                extra = f"  ({', '.join(bits)} live)" if bits else ""
                self.results.setText(f"deployed: {self.current_project}{extra}")
            else:
                self.results.setText(f"deploy failed: {resp.get('error', '?')}")
        except Exception as e:
