...
from home_screen import load_home_ui_settings, register_themed_screen

# --- n8n-style canvas background --------------------------------------------
CANVAS_BG_GREY = "#242424"   # flat grey default (n8n-ish dark canvas)
CANVAS_DOT_COLOR = "#3d3d3d"
CANVAS_DOT_SPACING = 24      # world-space spacing between dots
CANVAS_DOT_RADIUS = 1.4      # screen-space radius (stays constant across zoom)


def paint_canvas_background(widget, painter, settings):
    """Screen-space flat/image background fill. Shared by the Canvas itself
    and by editor.py's side panels, so they match the canvas's background
    (grey, image, or nothing) instead of looking inconsistent next to it."""
    bg_path = settings.get("canvas_bg_image")
    ...
    if pix and not pix.isNull():
        ...
        painter.fillRect(widget.rect(), QColor(CANVAS_BG_GREY))
...
class Canvas(QWidget):
    ...
    def apply_theme(self):
        """Called by the settings broadcast when canvas/workflow settings
        are saved elsewhere — just needs a repaint, settings are already
        swapped in by the broadcaster."""
        self.update()
    ...
    def _paint_flat_or_image_bg(self, p):
        """Screen-space background fill: canvas bg image (cover, dimmed for
        readability) or flat grey. Skipped entirely if canvas_no_background
        is on — caller checks that first."""
        paint_canvas_background(self, p, self.settings)

    def _paint_dot_grid(self, p):
        """n8n-style dot grid, drawn in world space (after translate/scale)
        so it pans and zooms with the canvas content. Dot radius is scaled
        inversely so dots stay a constant size on screen."""
        top_left = self.world(QPointF(0, 0))
        bottom_right = self.world(QPointF(self.width(), self.height()))
        spacing = CANVAS_DOT_SPACING
        x0 = int(top_left.x() // spacing) * spacing
        y0 = int(top_left.y() // spacing) * spacing
        r = max(0.4, CANVAS_DOT_RADIUS / max(self.scale, 0.05))
        p.setPen(Qt.PenStyle.NoPen)
        p.setBrush(QBrush(QColor(CANVAS_DOT_COLOR)))
        y = y0
        while y <= bottom_right.y() + spacing:
            x = x0
            while x <= bottom_right.x() + spacing:
    ...
    def paintEvent(self, _):
        ...
        no_bg = self.settings.get("canvas_no_background", False)
        if not no_bg:
            self._paint_flat_or_image_bg(p)
        p.translate(self.offset)
        p.scale(self.scale, self.scale)
        if not no_bg and self.settings.get("canvas_dots", True):
            self._paint_dot_grid(p)
        for i, (src, oi, dst, ii) in enumerate(self.connections):
